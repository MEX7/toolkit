...
func InitStream(client pb.StreamClient, callback Callback) *ProxyStream {
	obj := &ProxyStream{msgChan: make(chan *pb.CellReq, 1000)}
	obj.PushChan(&pb.CellReq{MsgId: MsgConnectReq})
	obj.sync(client, callback)
	return obj
}
...
func (p *ProxyStream) sync(client pb.StreamClient, callback Callback) {
	xgo.Go(func() {
		var (
			reply *pb.CellResp
			err   error
